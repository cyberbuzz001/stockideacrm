@props(['title', 'value', 'icon' => 'chart-bar', 'color' => 'indigo', 'link' => null])

<div
    class="bg-white p-6 rounded-3xl shadow-sm border border-slate-100 flex items-center justify-between group hover:shadow-md transition-shadow relative">
    @if($link)
    ...
    <div
        class="w-12 h-12 rounded-full bg-{{ $color }}-100 flex items-center justify-center text-{{ $color }}-600 group-hover:scale-110 transition-transform">
        @if($icon === 'users')
